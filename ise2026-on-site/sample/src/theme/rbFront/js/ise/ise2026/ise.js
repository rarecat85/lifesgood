...
async function changeVideoSource(videoPath, shouldLoop) {
  ...
  // 새 영상이 로드되면 재생
  const handleCanPlay = () => {
    currentPlayPromise = kvVideo.play()
      .then(() => {
...
function init() {
  ...
  new fullpage('.ise-container', {
    ...
    navigation: true,
    navigationTooltips: ['Home', 'Wall graphic','Booth map','Hallyu store', 'LED Tech Zone','Highlight'],
    showActiveTooltip: true,
    scrollOverflow: true, // 콘텐츠가 넘칠 때 섹션 내부 스크롤 활성화
    normalScrollElements: '.layer-popup, .layer-popup *, .globe',
  });
